...
public interface TokenDatabase {
    ...
    final Token[] kTokenList = {
        ...
        // Operators
        new Operator(
            "and", (a, b) -> BoolToDouble(DoubleToBool(a) && DoubleToBool(b)), 0, Operator.Order.LEFT
        ),
        new Operator(
            "or",  (a, b) -> BoolToDouble(DoubleToBool(a) || DoubleToBool(b)), 0, Operator.Order.LEFT
        ),
        new Operator(
            "xor", (a, b) -> BoolToDouble(DoubleToBool(a) != DoubleToBool(b)), 0, Operator.Order.LEFT
        ),
        new UnaryFunction(
            "not", (a) -> BoolToDouble(!DoubleToBool(a))
        ),

        new Operator(
            "==", (a, b) -> BoolToDouble(a == b), 1, Operator.Order.LEFT
        ),
        new Operator(
            "!=", (a, b) -> BoolToDouble(a != b), 1, Operator.Order.LEFT
        ),
        ...
        new UnaryFunction(
            "sqrt", (in) -> Math.sqrt(in)
        ),
        new UnaryFunction(
            "cbrt", (in) -> Math.cbrt(in)
        ),
        new UnaryFunction(
            "sin", (in) -> Math.sin(in)
        ),
        ...
        new UnaryFunction(
            "log10", (in) -> Math.log10(in)
        ),
        new UnaryFunction(
            "log", (in) -> Math.log(in)
        ),
        new UnaryFunction(
            "signum", (in) -> (0 < in) ? 1.0 : -1.0
        ),
        new UnaryFunction(
            "bool", (in) -> BoolToDouble(DoubleToBool(in))
        ),
        new UnaryFunction(
            "round", (in) -> Math.round(in)
        ),
        new UnaryFunction(
            "int", (in) -> Math.floor(in)
        ),
        new UnaryFunction(
            "floor", (in) -> Math.floor(in)
        ),
        new UnaryFunction(
            "ceil", (in) -> Math.ceil(in)
        ),
